...
class FhirSchemaModel():
    ...
    def all_fhir_resource_mapping(cls):
        return { str(x).rsplit('/',1)[1][:-5] : StructType.fromJson(json.load(open(x, "r"))) for x in list(files("schemas").iterdir())}
    ...
    def us_core_fhir_resource_mapping(cls):
        ...
        return { x : StructType.fromJson(json.load(open(files('schemas').joinpath(x + ".json"), "r"))) for x in us_core_resources}
    ...
    def custom_fhir_resource_mapping(cls, resource_list):
        return { x : StructType.fromJson(json.load(open(files('schemas').joinpath(x + ".json"), "r"))) for x in resource_list}
