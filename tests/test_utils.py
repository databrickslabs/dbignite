...
class SparkTest:
    ...
    def setup_class(self) -> None:
        self.spark = (
            SparkSession.builder.appName("dbignite")
...
class TestUtils(SparkTest):
    ...
    def test_entries_to_procedure_occurrence(self) -> None:
        ...
        self.assertSchemasEqual(
            procedure_occurrence_df.schema, PROCEDURE_OCCURRENCE_SCHEMA
        )
    ...
    def test_summarize_condition(self) -> None:
        ...
        summarize_condition_df = entries_df.transform(entries_to_condition).transform(
            summarize_condition
        )
        summarize_condition_first = summarize_condition_df.first()
        assert summarize_condition_df.count() == 3
        assert (
                summarize_condition_first.person_id
                == "4a0bf980-a2c9-36d6-da55-14d7aa5a85d9"
        )
        assert (
                summarize_condition_first.conditions[0].condition_occurrence_id
                == "a6425765-fb3a-3b1a-161b-4b5ad0fe0495"
        )
        assert (
                summarize_condition_first.conditions[0].visit_occurrence_id
                == "a6118cdf-24ea-94fe-6a6e-30e91c841e5a"
        )
        assert (
                summarize_condition_first.conditions[0].condition_status == "Hypertension"
        )

    def test_summarize_procedure_occurrence(self) -> None:
        entries_df = Transformer(spark).load_entries_df(TEST_BUNDLE_PATH)
        summarize_procedure_occurrence_df = entries_df.transform(
            entries_to_procedure_occurrence
        ).transform(summarize_procedure_occurrence)
        ...
        assert (
                summarize_procedure_occurrence_first.person_id
                == "4a0bf980-a2c9-36d6-da55-14d7aa5a85d9"
        )
        assert (
                summarize_procedure_occurrence_first.procedure_occurrences[
                    0
                ].procedure_occurrence_id
                == "562ad62b-0671-c044-8dd1-4f60ad010bec"
        )
        assert len(summarize_procedure_occurrence_first) == 2

    def test_summarize_encounter(self) -> None:
        entries_df = Transformer(spark).load_entries_df(TEST_BUNDLE_PATH)
        summarize_encounter_df = entries_df.transform(entries_to_encounter).transform(
            summarize_encounter
        )
        ...
        assert (
                summarize_encounter_first.person_id
                == "4a0bf980-a2c9-36d6-da55-14d7aa5a85d9"
        )
        assert (
                summarize_encounter_first.encounters[1].encounter_id
                == "83075051-3b84-b92a-cefb-66497ee9d602"
        )
        assert (
                summarize_encounter_first.encounters[1].location[0].location.display
...
class TestTransformers(SparkTest):
    ...
    def test_fhir_bundles_to_omop_cdm(self) -> None:
        omop_cdm = Transformer(self.spark).fhir_bundles_to_omop_cdm(
            TEST_BUNDLE_PATH, TEST_DATABASE, None, True
        )
        tables = [
            t.tableName
            for t in self.spark.sql(f"SHOW TABLES FROM {TEST_DATABASE}").collect()
        ]
    ...
    def test_omop_cdm_to_person_dashboard(self) -> None:
        ...
        omop_cdm = transformer.fhir_bundles_to_omop_cdm(
            TEST_BUNDLE_PATH, TEST_DATABASE, None, True
        )
        person_dashboard = transformer.omop_cdm_to_person_dashboard(omop_cdm).summary()
        self.assertSchemasEqual(
            CONDITION_SUMMARY_SCHEMA, person_dashboard.select("conditions").schema
        )
